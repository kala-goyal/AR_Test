...
public class Gnat : Insect
{
    ...
    public override void SpawnSelf(GameObject bugPrefab, TrackableBehaviour parentObj)
    {
        ...
        _curZ = CreatureManagerScript.bulbZ;                // start bug at middle of bulb
    ...
    public override void Update(float time)
    {
        ...
        if (CreatureManagerScript.lightState)
        {
            // light is on
            if (_curRadius > CreatureManagerScript.maxRadius)
            {
            ...
            else
            {
                ...
                _curRadiusIncrement = Helpers.IncrementRanger(_curRadiusIncrement, _curRadius, CreatureManagerScript.minRadius, CreatureManagerScript.maxRadius);
        ...
        else
        {
            ...
            if (_curRadius > MAX_RADIUS)
            {
                ...
                CreatureManagerScript.RemoveCreature(this);
            }
        }
        _curRadius += _curRadiusIncrement;
        //Debug.Log(">>>>>>>> " + _curRadius);

        _curZincrement = Helpers.IncrementRanger(_curZincrement, _curZ, CreatureManagerScript.Zbot, CreatureManagerScript.Ztop);
